...
- **L1: 滚动摘要 (Context Refresh)**：自动压缩历史对话，按“今天、昨天、本周、更早”分级展示，在节省 Token 的同时保留长期上下文。
...
- **智能交互体验**：采用“隐形指令”技术。系统自动触发的指令对用户不可见，而 AI 执行完任务后的反馈则会正常显示并推送通知，确保流程清晰且不干扰主对话流。
