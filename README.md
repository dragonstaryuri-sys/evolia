...
### 2. 智能检索与重排 (RAG Optimization) 🆕
为了确保 AI 提取的记忆既精准又相关，我们实现了高效的双阶检索机制：
- **双阶过滤策略**：初阶“宽口径召回”确保覆盖面（召回数量为设定的 3 倍），二阶“严格精排”通过重排模型对相关性进行终审。
- **混合计分引擎**：支持向量语义（Semantic）与关键词（Keyword）混合匹配，并引入**时效性衰减（Recency Boost）**权重，优先唤醒近期发生的记忆。
- **硬性阈值屏障**：严格执行“综合评分阈值”过滤，重排后分数不达标的干扰信息将不会被送往 AI 上下文，彻底告别无关记忆的干扰。

### 3. 用户观察者与系统控制 (The Watcher)
