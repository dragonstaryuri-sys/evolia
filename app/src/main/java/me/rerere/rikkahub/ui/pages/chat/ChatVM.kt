...
class ChatVM(
    ...
    private fun initPaginationManager(assistantId: Uuid) {
        Log.d("PAGINATION_DEBUG", "VM.init: Manager initialized for assistant=$assistantId")
    ...
    init {
        viewModelScope.launch {
            ...
            // targetMessageId 跳转：初始化后循环 loadOlder 寻找目标消息
            if (!targetMessageId.isNullOrBlank()) {
                viewModelScope.launch {
                    val manager = paginationManager ?: return@launch
                    manager.state
                        .filterIsInstance<ConversationRepository.ChatPaginationState.Success>()
                        .collect { state ->
                            val found = state.nodes.any { node ->
                                node.messages.any { m -> m.id.toString() == targetMessageId }
                            }
                            if (found) {
                                this.coroutineContext[Job]?.cancel()
                            } else if (state.hasOlder) {
                                delay(100) // ✨ 增加 100ms 延迟，给数据库喘息的时间
                                manager.loadOlder()
                            } else {
                                this.coroutineContext[Job]?.cancel()
                            }
