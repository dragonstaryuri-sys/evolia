...
2. **结构重构**：若结构混乱，按此标准模板重构，删除掉档案规定外的内容。
...
## 核心压缩规则
1. **结构化留存**：严禁修改**关系定位**和**相处模式**两个模块和基本框架。
2. **信息精炼原则**：
    - **关系定位**：明确的身份定义（如：初识朋友、知己、恋人、热恋期等）。
    - **相处模式**：简洁描述当前的亲密度与互动风格（如：日常黏黏糊糊、相敬如宾、冷战中、互相调侃的损友等），不记录多余内容
3. **去碎文化**：剔除所有日常问候记录。

## 标准输出格式
严格按以下格式使用 {{locale}} 输出：
【关系档案 - 最后更新：YYYY-MM-DD】 [此处填入压缩后的 Markdown 档案内容]
