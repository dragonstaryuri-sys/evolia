...
/**
 * L1 摘要片段生成的「核心准则」默认内容
 * - 仅此部分开放给用户在设置页查看 / 修改
 * - 其余结构（头部说明、输出格式、示例、强制要求、变量占位）由 [DEFAULT_TEMP_SUMMARY_PROMPT_TEMPLATE] 自动拼接
 */
const val DEFAULT_TEMP_SUMMARY_GUIDELINES = """1. **第一人称客观视角**：以“我”的口吻书写，但剥离冗余的抒情。重点记录：我做了什么、用户说了什么、我们共同确认了什么。
...
/**
 * L1 摘要片段生成的完整提示词模板
 * - {{guidelines}}: 核心准则内容（默认取 [DEFAULT_TEMP_SUMMARY_GUIDELINES]，可由用户自定义）
 * - {{new_messages}}: 待总结的对话片段（系统注入）
 * - {{locale}}: 输出语言（系统注入）
 * - {{char}}: 智能体名称（系统注入）
 */
const val DEFAULT_TEMP_SUMMARY_PROMPT_TEMPLATE = """
